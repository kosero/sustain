...
static void draw_entity_node(CoreContext *ctx, ecs_entity_t e) {
  ...
  if (nk_tree_element_push_id(ctx->nk_ctx, NK_TREE_NODE, name, NK_MINIMIZED,
                              &is_selected, (int)e)) {
    ecs_iter_t children = ecs_children(ctx->world, e);
    while (ecs_children_next(&children)) {
      for (int i = 0; i < children.count; i++) {
        if (ecs_has_id(ctx->world, children.entities[i], ecs_id(GameObject))) {
          draw_entity_node(ctx, children.entities[i]);
        }
      }
    }
