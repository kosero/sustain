...
void editor_camera_update(EditorCamera *cam, bool is_hovered)
{
	...
	if (cam->is_moving) {
		...
		if (IsKeyDown(KEY_W)) {
			CameraMoveForward(&cam->camera, move_speed, false);
		}
		if (IsKeyDown(KEY_S)) {
			CameraMoveForward(&cam->camera, -move_speed, false);
		}
		if (IsKeyDown(KEY_D)) {
			CameraMoveRight(&cam->camera, move_speed, false);
		}
		if (IsKeyDown(KEY_A)) {
			CameraMoveRight(&cam->camera, -move_speed, false);
		}
		if (IsKeyDown(KEY_E)) {
			CameraMoveUp(&cam->camera, move_speed);
		}
		if (IsKeyDown(KEY_Q)) {
			CameraMoveUp(&cam->camera, -move_speed);
		}
		...
		CameraPitch(&cam->camera, -mouse_delta.y * 0.003f, true, false,
			    false);
	}
}
